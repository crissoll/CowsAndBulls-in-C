...
void cab_session__disable_command(CabSession* session,
    ...
    if (tree->disabled_commands == NULL) {
        tree->disabled_commands = malloc(sizeof(*tree->disabled_commands) * 4);
        tree->disabled_commands_allocated_size = 4;
        tree->disabled_commands_current_size = 1;
        tree->disabled_commands[0] = specifier;
        return;
    }
    if (cab_session__is_command_allowed(*session, specifier) == false) {
        return;
    }
    if (tree->disabled_commands_current_size >=
        tree->disabled_commands_allocated_size) {
        tree->disabled_commands_allocated_size *= 3;
        tree->disabled_commands_allocated_size /= 2;
        tree->disabled_commands = realloc(
            tree->disabled_commands, tree->disabled_commands_allocated_size);
    }
