...
static IndexArray filter__words_with_letter_anywhere(const WordSet* word_set,
                                                     size_t letter_idx) {
    IndexArray result;
...
IndexArray filter__get_words_from_word_set(const WordSet* word_set,
    ...
    /* For each position, union words with allowed letters, then intersect across
   * positions */
    for (size_t i = 0; i < LETTERS_IN_WORD; i++) {
    ...
    for (size_t letter_idx = 0; letter_idx < ALPHABET_SIZE; letter_idx++) {
        if (filter->required_letters[letter_idx]) {
            IndexArray has_letter =
                filter__words_with_letter_anywhere(word_set, letter_idx);
